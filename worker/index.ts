...
function validateExportData(data: unknown): { valid: boolean; errors: string[] } {
  ...
  // version/exportDate/configs 是 NaviHive 备份的附加字段，基础 groups/sites JSON 也应可导入。
  if (d.version !== undefined && typeof d.version !== 'string') errors.push('version 必须是字符串');
  if (d.exportDate !== undefined && typeof d.exportDate !== 'string') errors.push('exportDate 必须是字符串');
  ...
  // 验证 configs
  if (d.configs !== undefined && (!d.configs || typeof d.configs !== 'object' || Array.isArray(d.configs))) {
    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      try {
        ...
        // GET /api/groups 获取所有分组
          ...
          if (api.isAuthEnabled() && !isAuthenticated) {
            return createResponse('只有管理员可以删除分组', request, { status: 403 });
          }
          const idStr = path.split('/')[1];
          if (!idStr) {
            return createJsonResponse({ error: '无效的ID' }, request, { status: 400 });
        ...
        // 站点相关API
          ...
          return createJsonResponse(result, request);
        } else if (path === 'sites/batch' && method === 'DELETE') {
          if (api.isAuthEnabled() && !isAuthenticated) {
            return createResponse('只有管理员可以删除书签', request, { status: 403 });
          }
          const data = (await validateRequestBody(request)) as { ids?: unknown };
          if (!Array.isArray(data.ids) || data.ids.length === 0 || data.ids.length > 500) {
            return createJsonResponse({ success: false, message: 'ids 必须是 1-500 个数字组成的数组' }, request, {
              status: 400,
            });
          }
          const ids = data.ids.filter((id): id is number => Number.isInteger(id) && id > 0);
          if (ids.length !== data.ids.length) {
            return createJsonResponse({ success: false, message: 'ids 必须全部为正整数' }, request, { status: 400 });
          }
          const result = await api.deleteSites(ids);
          return createJsonResponse({ success: result, deleted: ids.length }, request);
        ...
        // 数据导入路由
          ...
          const importOptions =
            data && typeof data === 'object' && 'importOptions' in data
              ? (data as { importOptions?: unknown }).importOptions
              : undefined;
          const payload = data as ExportData & { importOptions?: unknown };
          const result = await api.importData(payload, importOptions as ImportOptions | undefined);
          return createJsonResponse(result, request);
        }
