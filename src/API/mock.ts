...
import { Group, Site, LoginResponse, ExportData, ImportResult, GroupWithSites, ImportOptions } from './http';
...
export class MockNavigationClient {
  ...
  async deleteSites(ids: number[]): Promise<boolean> {
    ...
    const before = mockSites.length;
    for (let index = mockSites.length - 1; index >= 0; index -= 1) {
      if (mockSites[index] && idSet.has(mockSites[index].id as number)) mockSites.splice(index, 1);
    }
    return mockSites.length < before;
  }
  ...
  // 数据导入
  async importData(data: ExportData, _options?: ImportOptions): Promise<ImportResult> {
    await new Promise((resolve) => setTimeout(resolve, 500));
    ...
    try {
      ...
      for (const importSite of data.sites) {
        ...
        // URL 全局判重，与 Cloudflare D1 实现保持一致，允许重新归类。
        const existingSiteIndex = mockSites.findIndex((s) => s.url === importSite.url);
