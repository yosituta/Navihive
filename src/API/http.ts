...
// 定义D1数据库类型 (compatible with Cloudflare Workers D1Database)
...
export class NavigationAPI {
  ...
  async deleteSites(ids: number[]): Promise<boolean> {
    const response = await this.request('sites/batch', {
      method: 'DELETE',
  ...
  async importData(data: ExportData, options?: ImportOptions): Promise<ImportResult> {
    try {
      const selectedGroupIds = options?.selectedGroupIds?.length
        ? new Set(options.selectedGroupIds)
        : null;
      const duplicateMode = options?.duplicateMode ?? 'update';
      const sourceGroups = selectedGroupIds
        ? data.groups.filter((group) => group.id !== undefined && selectedGroupIds.has(group.id))
        : data.groups;
      const sourceSites = selectedGroupIds
        ? data.sites.filter((site) => selectedGroupIds.has(site.group_id))
        : data.sites;
      ...
      for (const site of sourceSites) {
        ...
        // URL 是全局唯一的导入判定依据，避免重复导入时无法将站点归入新分组。
        const existingSite = await this.getSiteByUrl(site.url);
  ...
  async getSiteByUrl(url: string): Promise<Site | null> {
    const result = await this.db
      .prepare(
        'SELECT id, group_id, name, url, icon, description, notes, order_num, created_at, updated_at FROM sites WHERE url = ? LIMIT 1'
      )
      .bind(url)
      .first<Site>();
    return result;
