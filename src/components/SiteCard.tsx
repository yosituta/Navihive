...
interface SiteCardProps {
  ...
  selected?: boolean;
  onSelect?: (siteId: number, selected: boolean) => void;
}
...
const SiteCard = memo(function SiteCard({
  ...
  const handleSelectClick = (e: React.ChangeEvent<HTMLInputElement>) => {
    e.stopPropagation();
    if (site.id !== undefined) onSelect?.(site.id, e.target.checked);
  };
      ...
      >
        {selectionMode && (
          <input
            ...
            style={{ position: 'absolute', top: 8, left: 8, zIndex: 4, width: 18, height: 18 }}
          />
        )}
