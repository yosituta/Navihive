...
interface GroupCardProps {
  ...
  onVisitSite?: (site: Site) => void;
  showManagementActions?: boolean;
  selectionMode?: boolean;
  selectedSiteIds?: number[];
  onSelectSite?: (siteId: number, selected: boolean) => void;
}
                ...
                {sitesToRender.map((site, idx) => (
                  ...
                  >
                    <SiteCard
                      ...
                      onVisit={onVisitSite}
                      selectionMode={selectionMode}
                      selected={site.id !== undefined && selectedSiteIds.includes(site.id)}
      ...
      sx={{
        ...
        background: (theme) =>
          theme.palette.mode === 'dark'
            ? 'linear-gradient(180deg, rgba(11, 26, 37, 0.96), rgba(7, 19, 29, 0.92))'
      ...
      >
        <Box sx={{ flex: 1, minWidth: 0 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mt: 1.25 }}>
            <Chip
              size='small'
              label={group.is_public === 0 ? '私密分组' : '公开分组'}
              color={group.is_public === 0 ? 'default' : 'primary'}
              variant={group.is_public === 0 ? 'outlined' : 'filled'}
            />
            <Chip size='small' label={`${group.sites.length} 个站点`} variant='outlined' />
          </Box>
        </Box>
        ...
        >
          {isCurrentEditingGroup ? (
            ...
            sortMode === 'None' &&
            viewMode === 'edit' &&
            showManagementActions && ( // 只在编辑模式显示按钮
              <>
                {onAddSite && group.id && (
                  <Button
                    variant='contained'
