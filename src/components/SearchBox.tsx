...
const SearchBox: React.FC<SearchBoxProps> = ({ groups, sites, onInternalResultClick }) => {
  ...
  useEffect(() => {
    if (mode === 'internal') {
      if (!query.trim()) {
        // History is opened only by an explicit search-box focus. Keeping it
        // closed here prevents unrelated menu/dialog state changes from
        // making the history panel reappear while the query is empty.
        setShowHistory(false);
        setShowResults(false);
        return;
      }
  ...
  const handleInputFocus = () => {
    if (suppressFocusHistoryRef.current) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      const historyPanel = (target as HTMLElement)?.closest?.('[data-search-history-panel]');
      const resultsPanel = (target as HTMLElement)?.closest?.('[data-search-results-panel]');
      if (
        ...
        suppressFocusHistoryRef.current = true;
        window.setTimeout(() => {
          suppressFocusHistoryRef.current = false;
        }, 0);
        setShowResults(false);
        setShowHistory(false);
        // 菜单、按钮等外部控件被点击时，主动移除搜索框焦点，避免浏览器回焦后重新打开历史。
        inputRef.current?.blur();
      }
    };

    // 使用捕获阶段，确保 MUI Menu/Dialog 的 Portal 或 stopPropagation 不会阻止关闭历史面板。
