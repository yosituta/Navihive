...
function isExportData(value: unknown): value is ExportData {
  ...
  return Array.isArray(value.groups) && Array.isArray(value.sites);
}

function normalizeExportData(value: Record<string, unknown>): ExportData {
  const groups = (value.groups as Group[]).map((group, index) => ({
    id: group.id ?? index + 1,
    name: sanitizeName(group.name, `未命名分组 ${index + 1}`),
    order_num: typeof group.order_num === 'number' ? group.order_num : index,
    is_public: group.is_public ?? 1,
  }));
  const validIds = new Set(groups.map((group) => group.id));
  const sites = (value.sites as Site[])
    .filter((site) => validIds.has(site.group_id) && isSupportedBookmarkUrl(site.url))
    .map((site, index) => ({
      id: undefined,
      group_id: site.group_id,
      name: sanitizeName(site.name, getUrlFallbackName(site.url)),
      url: site.url.trim(),
      icon: site.icon || '',
      description: site.description || '',
      notes: site.notes || '',
      order_num: typeof site.order_num === 'number' ? site.order_num : index,
      is_public: site.is_public ?? 1,
    }));
  return {
    groups,
    sites,
    configs: isRecord(value.configs) ? (value.configs as Record<string, string>) : {},
...
function parseJsonImport(rawText: string): PreparedImportData {
  ...
  if (isExportData(parsed)) {
    const data = normalizeExportData(parsed);
    return {
      data,
      summary: {
