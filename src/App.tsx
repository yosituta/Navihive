...
import { prepareImportDataFromText, type PreparedImportData } from './utils/bookmarkImport';
...
function App() {
  ...
  const [selectedImportGroupIds, setSelectedImportGroupIds] = useState<number[]>([]);
  const [openExport, setOpenExport] = useState(false);
  const [selectedExportGroupIds, setSelectedExportGroupIds] = useState<number[]>([]);
  ...
  const handleSiteSelect = (siteId: number, selected: boolean) => {
    setSelectedSiteIds((current) =>
      selected ? Array.from(new Set([...current, siteId])) : current.filter((id) => id !== siteId)
    );
  };

  const handleBatchDelete = async () => {
    if (!isAuthenticated || selectedSiteIds.length === 0) return;
    if (!window.confirm(`确定删除选中的 ${selectedSiteIds.length} 个书签吗？`)) return;
    try {
      await api.deleteSites(selectedSiteIds);
      setSelectedSiteIds([]);
      setSelectionMode(false);
      await fetchData();
    } catch (error) {
      handleError('批量删除失败: ' + (error as Error).message);
    }
  };
  ...
  const handleExportData = async () => {
    setSelectedExportGroupIds(groups.map((group) => group.id));
    setOpenExport(true);
  };
  ...
  const confirmExportData = async () => {
    try {
      ...
      // 提取所有站点数据为单独的数组
      const allSites: Site[] = [];
      const selectedIds = new Set(selectedExportGroupIds);
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (selectedFile) {
        ...
        setImportError(null);
        try {
          const parsed = prepareImportDataFromText(selectedFile.name, await selectedFile.text());
          setPreparedImport(parsed);
          setSelectedImportGroupIds(parsed.data.groups.map((group) => group.id as number));
        } catch (error) {
          setPreparedImport(null);
  ...
  const handleImportData = async () => {
    ...
    try {
      ...
      setImportError(null);
      const parsedImport = preparedImport ?? prepareImportDataFromText(importFile.name, await importFile.text());
      const selectedIds = new Set(
        (selectedImportGroupIds.length > 0
          ? selectedImportGroupIds
          : parsedImport.data.groups.map((group) => group.id as number))
      );
      const filteredGroups = parsedImport.data.groups.filter((group) => selectedIds.has(group.id as number));
      const filteredSites = parsedImport.data.sites.filter((site) => selectedIds.has(site.group_id));
      if (filteredGroups.length === 0) throw new Error('请至少选择一个分组');

      const result = await api.importData(
        { ...parsedImport.data, groups: filteredGroups, sites: filteredSites },
      ...
      const summary = [
        `导入成功，已保存到数据库。`,
        `识别格式：${parsedImport.summary.formatLabel}`,
        `解析结果：分组 ${filteredGroups.length} 个，书签 ${filteredSites.length} 个`,
      ];

      if (parsedImport.summary.skippedCount > 0) {
        summary.push(`本地预处理：跳过 ${parsedImport.summary.skippedCount} 个非网页收藏项`);
      }
        ...
        >
                    ...
                    <>
                      <Button
                        ...
                        onClick={() => {
                          ...
                          if (selectionMode) setSelectedSiteIds([]);
                        }}
                      >
                        {selectionMode ? '取消选择' : '批量删除'}
                      </Button>
                      {selectionMode && (
                        <Button
                          variant='contained'
          ...
          >
            ...
            <DialogContent>
              ...
              {preparedImport && (
                <Box sx={{ mb: 2, border: '1px solid', borderColor: 'divider', p: 1.5 }}>
                  <Typography variant='subtitle2'>选择要导入的分组</Typography>
                  {preparedImport.data.groups.map((group) => {
                    const groupId = group.id as number;
                    return (
                      <Box key={groupId} sx={{ display: 'flex', alignItems: 'center' }}>
                        <Checkbox
                          size='small'
                          checked={selectedImportGroupIds.includes(groupId)}
                          onChange={(event) =>
          ...
          <Dialog open={openExport} onClose={() => setOpenExport(false)} maxWidth='sm' fullWidth>
            <DialogTitle>选择导出分组</DialogTitle>
            <DialogContent>
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 1 }}>
                <Button size='small' onClick={() => setSelectedExportGroupIds(groups.map((group) => group.id))}>全选</Button>
                <Button size='small' onClick={() => setSelectedExportGroupIds([])}>清空</Button>
              </Box>
              {groups.map((group) => (
                <Box key={group.id} sx={{ display: 'flex', alignItems: 'center' }}>
                  <Checkbox
                    size='small'
                    checked={selectedExportGroupIds.includes(group.id)}
                    onChange={(event) =>
                      setSelectedExportGroupIds((current) =>
                        event.target.checked
                          ? [...current, group.id]
            ...
            <DialogActions>
              <Button onClick={() => setOpenExport(false)}>取消</Button>
              <Button onClick={confirmExportData} variant='contained' disabled={selectedExportGroupIds.length === 0}>
                导出选中分组
              </Button>
            </DialogActions>
          </Dialog>
